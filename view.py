...
@app.route("/findString", methods=["POST"])
def findString():
    id = request.form["id"]
    with sqlite3.connect("app.db") as con:
        try:
            con.row_factory = sqlite3.Row
            cur = con.cursor()
            cur.execute("SELECT * FROM tablenso WHERE id_tip = " + "'" + id + "' or index_tip = " + "'" + id + "' or name_tip_podtip = " + "'" + id + "'")
            rows = cur.fetchall()
        except:
            rows = "Упс, что-то пошло не так :("
        finally:
            return render_template("allTable.html", rows=rows)


@app.route("/findStringXoz", methods=["POST"])
def findStringXoz():
    id = request.form["id"]
    with sqlite3.connect("app.db") as con:
        try:
            con.row_factory = sqlite3.Row
            cur = con.cursor()
            cur.execute("SELECT * FROM xoz WHERE ID_type = " + "'" + id + "' or ID_soil = " + "'" + id + "' or podtype = " + "'" + id + "'")
            rows = cur.fetchall()
        except:
            rows = "Упс, что-то пошло не так :("
